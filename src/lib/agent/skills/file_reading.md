...
3. **Read just enough to answer the user's question.** If they asked
   ...
   embedded newlines).
4. **Do not hallucinate from filenames.** A file path or file name is not evidence of its contents. Inspect schema/content before claiming metrics, rows, columns, names, dates, or totals.
5. **If a large file blocks full reading, continue with a sampled/profiled workflow.** State what was sampled, what remains unread, and use scripts/tools to process the full file when the exact result requires it.
6. **If a dedicated skill exists, go read it.** The table below tells
